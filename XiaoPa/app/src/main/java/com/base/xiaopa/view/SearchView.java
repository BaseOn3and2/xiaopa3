package com.base.xiaopa.view;

/**
 * Created by cldn1 on 2017/10/4.
 * 自定义搜索编辑框
 */
...
public class SearchView extends EditText {
    ...
    private void DrawSearchIcon(Canvas canvas) {
        if (this.getText().toString().length() == 0) {
            ...
            canvas.drawText("搜索", getScrollX() + searchSize + 8, getScrollY() +
                    (getHeight() - (getHeight() - textHeight) / 2) - paint.getFontMetrics().bottom
                    - dy, paint);
            canvas.restore();
        }
    }
